...
def lpfilter(filt_type, img):
    ...
    res_img = {
        "ideal": ifft2(F * np.reshape((d <= d0).astype(np.uint8), F.shape)),
        "butterworth": ifft2(F * np.reshape(1 / (1 + (d / d0) ** 8), F.shape)),
        "gaussian": ifft2(F * np.reshape(np.exp(-(d ** 2) / (2 * d0 ** 2)), F.shape)),
    }
...
def colorize(im, all=False, fused=False):
    ...
    c_clut = [
        clut_p.iloc[1:, 8 : 8 + 3].values,
        clut_p.iloc[1:, 4 : 4 + 3].values,
        clut_p.iloc[1:, :3].values,
    ]
    ...
    res = np.array(
        [[clut[imfused[i, j], :, choice_v1[i, j]] for j in range(c)] for i in range(r)]
    )
